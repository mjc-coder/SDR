...
public:
    ...
    // INPUT 1 - Read Data
    // INPUT 2 - Pulse SHAPE
    void convolve(const data_type* input1, size_t length1, const data_type* input2, size_t length2, data_type* output, size_t lengthOutput, size_t ds)
    {
        ...
        for(size_t i = ds-1, outputIndex = 0; i < length1; i+=ds, outputIndex++)
        {
            for (size_t j = 0; j < length2; j++) 
            {
                output[outputIndex] += real_data[i + length2 - j] * input2[length2 - j - 1];
            }
        }
    ...
    size_t demodulate(data_type* real, data_type* imag, uint8_t* output, size_t number_of_points, size_t ds)
    {
        ...
        convolve(real, number_of_points, WAVE1, ds, op1, number_of_points,ds);
        convolve(real, number_of_points, WAVE2, ds, op2, number_of_points,ds);


        for(size_t i = 0; i < number_of_points; i++)
        {
            output[i] = (op1[i] > op2[i]) ? 1:0;
        }

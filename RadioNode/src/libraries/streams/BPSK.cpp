...
const RADIO_DATA_TYPE BPSK::LUT_REAL[2] = { 1, -1};
const RADIO_DATA_TYPE BPSK::LUT_180[2] = {1, 0};

// MATLAB rcosine(1/8,1,'sqrt',0.5,8)
const RADIO_DATA_TYPE BPSK::RRC_COEFF[] =    {   -0.000882663055055987,  -0.000746201590523265, -0.000336789968798916,  0.000225038893553655,
...
, UW_SIZE(UniqueWordLength)
{
    ...
    memset(&PhaseLoopFilterVariables, 0, sizeof(PhaseLoopFilterVars));
    PhaseCorrectionVariables.dds_real = 1;

    // Configure Unique Words
    if(UW_SIZE > 64) return; // Size check
    memset(uw_0, 0, 64*sizeof(RADIO_DATA_TYPE));
    memset(uw_180, 0, 64*sizeof(RADIO_DATA_TYPE));

    for(int i = UW_SIZE-1; i >= 0; i--)
    {
        // Shift and mask bits in reverse order... seems weird because of the memory layouts
        uint64_t mask = 1LL << i;
        uw_0[i] = ((UniqueWord & mask) > 0) ? 1 : 0;
        uw_180[i] = ((UniqueWord & mask) > 0) ? 0 : 1;
    }
}
...
void BPSK::modulate(uint8_t* data_in, size_t length, RADIO_DATA_TYPE* modulated_real, size_t length_modulated)
{
    m_RealData = new RADIO_DATA_TYPE[length*8];
    memset(m_RealData, 0, sizeof(RADIO_DATA_TYPE)*length);

    // LUT convert 0,1 to 1,-1
    encode_LUT(data_in, length);

    // Pulse Shape
    encode_pulseShape(modulated_real, length_modulated);
...
void BPSK::encode_LUT(uint8_t* data_in, size_t length)
{
    m_length = length;

    for(size_t index = 0; index < m_length; index++)
    {
        m_RealData[index] = LUT_REAL[data_in[index]];
    }
}


void BPSK::encode_pulseShape(RADIO_DATA_TYPE* output_data, size_t output_length)
{
    if(output_length < 8*m_length)
    {
        std::cout << "ERROR:: QPSK modulation output arrays are not large enough!!! [" << output_length << " vs " << 8*m_length<< std::endl;
        return;
    }

    memset(output_data, 0, output_length);
    RADIO_DATA_TYPE* data = new RADIO_DATA_TYPE[(m_length*8)+(129*2)];  // Upsample 8, and include forward buffer for FIR transport
    memset(data, 0, ((m_length*8)+(129*2))*sizeof(RADIO_DATA_TYPE));

    output_length = upsample_fill_w_zeros(m_RealData, m_length, &data[129], m_length*8, 8);

    for(int i = 0; i < output_length; i++)
    {
        for(int j = 0; j < 129; j++)
        {
            output_data[i] += data[i+129-j] * RRC_COEFF[129-j-1];
        }
    }
...
void BPSK::demodulate(BBP_Block* input, BBP_Block* output)
{
    RADIO_DATA_TYPE* real_data = new RADIO_DATA_TYPE[input->number_of_points()];
    RADIO_DATA_TYPE* imag_data = new RADIO_DATA_TYPE[input->number_of_points()];

    memset(real_data, 0, input->number_of_points()*sizeof(RADIO_DATA_TYPE));
    memset(imag_data, 0, input->number_of_points()*sizeof(RADIO_DATA_TYPE));

    // RX Correlator
    size_t num_of_points = decode_pulseShape(input, real_data, imag_data);

    symbol_timing_synchronization(real_data, imag_data, num_of_points);

    num_of_points = phase_shift_correction(real_data, imag_data, num_of_points); // 1's and 0's after this

    phase_ambiguity_correction(real_data, imag_data, num_of_points);

    output->reset();
    // load the output block
    for(int i = 0; i < num_of_points; i++)
    {
        output->points[i].real(real_data[i]);
        output->points[i].imag(imag_data[i]);
        output->n_points++;
    }
...
size_t BPSK::demodulate(Complex_Array& input, uint8_t* output, size_t number_of_points, size_t downsample, RADIO_DATA_TYPE* real_plot_data, RADIO_DATA_TYPE* imag_plot_data)
{
    // need to downsample before we demodulate... transmitter upsamples the bits
    size_t points = decimateComplexArray(input, number_of_points, downsample);

    RADIO_DATA_TYPE* real_data = new RADIO_DATA_TYPE[points];
    RADIO_DATA_TYPE* imag_data = new RADIO_DATA_TYPE[points];

    memset(real_data, 0, points*sizeof(RADIO_DATA_TYPE));
    memset(imag_data, 0, points*sizeof(RADIO_DATA_TYPE));

    // RX Correlator
    size_t num_of_points = decode_pulseShape(input, points, real_data, imag_data);

    symbol_timing_synchronization(real_data, imag_data, num_of_points);

    memcpy(real_plot_data, real_data, num_of_points*sizeof(RADIO_DATA_TYPE)); // should be a mix
    memcpy(imag_plot_data, imag_data, num_of_points*sizeof(RADIO_DATA_TYPE)); // should be a mix

    num_of_points = phase_shift_correction(real_data, imag_data, num_of_points);
...
size_t BPSK::decode_pulseShape(Complex_Array& input, size_t number_of_points, RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* imag)
{
    RADIO_DATA_TYPE* real_data = new RADIO_DATA_TYPE[number_of_points+(129*2)];  // Upsample 8, and include forward buffer for FIR transport
    RADIO_DATA_TYPE* imag_data = new RADIO_DATA_TYPE[number_of_points+(129*2)];  // Upsample 8, and include forward buffer for FIR transport
    RADIO_DATA_TYPE* real_data_interp = new RADIO_DATA_TYPE[number_of_points];   // Upsample 8, and include forward buffer for FIR transport
    RADIO_DATA_TYPE* imag_data_interp = new RADIO_DATA_TYPE[number_of_points];   // Upsample 8, and include forward buffer for FIR transport

    memset(real_data, 0, (number_of_points+(129*2))*sizeof(RADIO_DATA_TYPE));
    memset(imag_data, 0, (number_of_points+(129*2))*sizeof(RADIO_DATA_TYPE));
    memset(real_data_interp, 0, number_of_points*sizeof(RADIO_DATA_TYPE));
    memset(imag_data_interp, 0, number_of_points*sizeof(RADIO_DATA_TYPE));

    // Initialize data
    for(int i = 0; i < number_of_points; i++)
    {
        real_data[i+129] = input[i].real();
        imag_data[i+129] = input[i].imag();
    }
    ...
    // Downsample by 4
    (void)decimate(real_data_interp, number_of_points, 4);
    (void)decimate(imag_data_interp, number_of_points, 4);

    memcpy(real, real_data_interp, sizeof(RADIO_DATA_TYPE)*number_of_points/4);
    memcpy(imag, imag_data_interp, sizeof(RADIO_DATA_TYPE)*number_of_points/4);

    delete[] real_data_interp;
    delete[] imag_data_interp;
    delete[] real_data;
    delete[] imag_data;
    return number_of_points/4;
}



size_t BPSK::decode_pulseShape(BBP_Block* input, RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* imag)
{
    memset(real, 0, (input->number_of_points()/4)*sizeof(RADIO_DATA_TYPE));
    memset(imag, 0, (input->number_of_points()/4)*sizeof(RADIO_DATA_TYPE));
    ...
    memcpy(real, real_data_interp, sizeof(RADIO_DATA_TYPE)*input->number_of_points()/4);
    memcpy(imag, imag_data_interp, sizeof(RADIO_DATA_TYPE)*input->number_of_points()/4);

    delete[] real_data_interp;
    delete[] imag_data_interp;
    delete[] real_data;
    delete[] imag_data;
    return input->number_of_points()/4;
}

void BPSK::symbol_timing_synchronization(RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* imag, size_t num_of_points)
{
    ...
    for(int i = 0; i < num_of_points; i++)
    {
        ...
        SymbolTimingSynchronizationVariables.Q = InterpolationFilter(Interp_Q_Channel, real[i], SymbolTimingSynchronizationVariables.mu);
        SymbolTimingSynchronizationVariables.I = InterpolationFilter(Interp_I_Channel, imag[i], SymbolTimingSynchronizationVariables.mu);
        ...
        real[i] = StrobeEnabledRegister(SymbolTimingSynchronizationVariables.Q, strobeTemp, Register_Q);
        imag[i] = StrobeEnabledRegister(SymbolTimingSynchronizationVariables.I, strobeTemp, Register_I);
    }
}

RADIO_DATA_TYPE BPSK::InterpolationFilter(InterpData& data, RADIO_DATA_TYPE in, RADIO_DATA_TYPE mu)
{
...
RADIO_DATA_TYPE BPSK::GardnerDetector(RADIO_DATA_TYPE i, RADIO_DATA_TYPE q, RADIO_DATA_TYPE strobe)
{
...
RADIO_DATA_TYPE BPSK::LoopFilter(RADIO_DATA_TYPE val)
{
...
RADIO_DATA_TYPE BPSK::NcoControl(RADIO_DATA_TYPE input)
{
...
RADIO_DATA_TYPE BPSK::StrobeEnabledRegister(RADIO_DATA_TYPE input, RADIO_DATA_TYPE strobe, RADIO_DATA_TYPE& reg)
{
    if(strobe >= 1)
    {
...
size_t BPSK::phase_shift_correction(RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* imag, size_t num_of_points)
{
    RADIO_DATA_TYPE temp_real = 0;
    RADIO_DATA_TYPE temp_imag = 0;
    const RADIO_DATA_TYPE constant = 8.0;
    RADIO_DATA_TYPE temp = 0;
    for(size_t i = 0; i < num_of_points; i++)
    {
        ...
        complex_multiply(real[i], imag[i], PhaseCorrectionVariables.dds_real, PhaseCorrectionVariables.dds_imag, temp_real, temp_imag);
        temp = PhaseLoopFilter(constant * (SIGN_FUNC(temp_imag)*temp_real - SIGN_FUNC(temp_real)*temp_imag));
        // Generate Output
        real[i] = PhaseShiftCorrection_OutputLookup(SIGN_FUNC(temp_real), SIGN_FUNC(temp_imag));
        imag[i] = 0;
    }

    return decimate(real, num_of_points, 2); // Rx correlated data is decimated by 2
}


void BPSK::complex_multiply(RADIO_DATA_TYPE ra, RADIO_DATA_TYPE ia, RADIO_DATA_TYPE rb, RADIO_DATA_TYPE ib, RADIO_DATA_TYPE& rc, RADIO_DATA_TYPE& ic)
{
    rc = ra*rb - ia*ib;
    ic = ia*rb + ra*ib;
}
...
RADIO_DATA_TYPE BPSK::PhaseLoopFilter(RADIO_DATA_TYPE val)
{
...
RADIO_DATA_TYPE BPSK::PhaseShiftCorrection_OutputLookup(RADIO_DATA_TYPE real, RADIO_DATA_TYPE imag)
{
    if(real < 0)
    {
        return 1;
...
void BPSK::phase_ambiguity_correction(RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* /*imag*/, size_t num_of_points)
{
    // Check ambiguity
    for(size_t i = 0; i < num_of_points-UW_SIZE; i++)
    {
        size_t match_count_0 = 0;
        size_t match_count_180 = 0;
        // check
        for(size_t j = 0; j < UW_SIZE; j++)
        {
            if(real[i+j] == uw_0[UW_SIZE - 1 - j])
            {
                match_count_0++;
            }
            if(real[i+j] == uw_180[UW_SIZE - 1 - j])
            {
                match_count_180++;
            }
        }

        if(match_count_0 == UW_SIZE)
        {
            --m_phase_ambiguity_cc;
            std::cout << "Phase 0" << std::endl;
        }
        else if(match_count_180 == UW_SIZE)
        {
            ++m_phase_ambiguity_cc;
            std::cout << "Phase 180" << std::endl;
        }
    }

    if(m_phase_ambiguity_cc.high_confidence())
    {
        // we have 180 Degree inversion
        for(size_t i = 0; i < num_of_points; i++)
        {
            real[i] = LUT_180[(int)real[i]];
        }
    }
    // Low confidence means the data is correct already
}

...
const RADIO_DATA_TYPE BPSK::LUT_180[] = {1, 0};
...
BPSK::BPSK(SafeBufferPoolQueue* BBP_Buffer_Pool,
           std::string name,
           std::string address,
...
, m_length(0)
, normalization_max(0)
, fout("bpsk.raw", std::ios::binary | std::ios::out | std::ios::trunc)
...
void BPSK::modulate(uint8_t* data_in, size_t length, RADIO_DATA_TYPE* modulated_real, size_t length_modulated)
{
    ...
    fout.write((const char*)m_RealData, sizeof(RADIO_DATA_TYPE)*(m_length));
...
void BPSK::demodulate(BBP_Block* input, BBP_Block* output)
{
    ...
    phase_shift_correction(real_data, imag_data, num_of_points);
...
void BPSK::phase_shift_correction(RADIO_DATA_TYPE* real, RADIO_DATA_TYPE* imag, size_t num_of_points)
{
    ...
    decimate(real, num_of_points, 2); // Rx correlated data is decimated by 2
}
